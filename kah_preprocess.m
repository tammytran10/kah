...
info = kah_info;
subject    = info.subj{2}; % change this to read in different datasets;  which subjects have which sessions/experiments are described in .../protocols/r1.json
experiment = 'FR1';    % change this to read in different datasets;  only FR1/2 can be segmented by rmr_upennram_trialfun
sessnum    = 1;      % change this to read in different datasets;  session numbers are zero-indexed
...
cfg = [];
cfg.channel = setdiff(data.label, infocurr.badchan.broken);
...
%%
[freq, medianpsd] = util_medianwelch(data.trial{1}(1,:), 2 * , 1600, 3200, data.fsample, []);
